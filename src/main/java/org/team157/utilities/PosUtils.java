...
public interface PosUtils {
    ...
    static boolean isOscillating(
        ...
        if ((currentPos >= desiredPos - posTolerance)
                && (currentPos <= desiredPos + posTolerance)) {
     ...
     * Note that while the arduino version uses integers, the calculation is the same for doubles,
     * so the warning about integer math is not applicable here.
     *
