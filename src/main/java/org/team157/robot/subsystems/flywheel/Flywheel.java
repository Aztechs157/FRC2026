...
public class Flywheel extends SubsystemBase {
    ...
    /**
     * Set the flywheel to a dynamically-calculated velocity based on the current distance and
    ...
    static double velocityFunction(double distance, double height, double theta) {
        ...
        double denominator =
                2
                        * Math.cos(theta)
                        * Math.cos(theta)
                        * (distance * Math.tan(theta) - heightDifference);
        if (denominator <= 0) {
     ...
     *
     * @param isUnderTrench Whether or not the turret is currently under the trench, determined by
     *     the drivetrain's position on the field.
    ...
    /**
     * Calculates and updates the optimal flywheel velocity and hood angle for the current shot
     * based on target distance and height.
     *
     ...
     * time it is called. The result of this calculation is multiplied by a modifier controlled by
     * the operator.
     *
